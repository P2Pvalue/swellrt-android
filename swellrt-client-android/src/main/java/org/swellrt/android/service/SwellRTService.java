...
public class SwellRTService extends Service implements UnsavedDataListener {
  ...
  private class LoginTask extends AsyncTask<String, Void, WaveHttpLogin.Response> {

    @Override
    protected WaveHttpLogin.Response doInBackground(String... params) {
      String sessionId = null;
      WaveHttpLogin login = new WaveHttpLogin(params[0], params[1], params[2]);
      WaveHttpLogin.Response response = login.execute();
      return response;
    }

    @Override
    protected void onPostExecute(WaveHttpLogin.Response result) {
      if (result != null) {
        participantId = ParticipantId.ofUnsafe(result.userId);
        openWebSocket(participantId.getDomain(), result.sessionId);
      } else {
  ...
  private void openWebSocket(String waveDomain, final String sessionId) {
    ...
    // TODO (pablojan) Check IdGenerator implementation, compare seed with GWT version
